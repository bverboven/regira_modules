...
class TreeList {
  ...
  addRange(
    values,
    { parents = [], position, fkSelector = this._fkSelector, parentsSelector = this._parentsSelector, valueSelector = this._valueSelector } = {}
  ) {
    ...
    const addedNodes = toArray(values).map((value, i) => {
      ...
      return parentNodes.map((parent) => {
        ...
        node.children = treeNodes.filter((child) => (fkSelector ? fkSelector(child.value, value) : (parentsSelector(child.value) || []).includes(value)));
        return node;
      });
    });
  ...
  moveRoot(value, position) {
    ...
    nodes.reverse();
    for (const node of nodes) {
      position == null ? treeNodes.push(node) : treeNodes.splice(position, 0, node);
    }
